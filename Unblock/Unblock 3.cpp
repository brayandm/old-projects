    ...
    while(!Q.empty())
    {
        ...
        for(int i = 0 ; i < ini.state.est.size() ; i++)
        {
            if(ini.state.est[i].p1.x == ini.state.est[i].p2.x)
            {
                ...
                for(int j = 1 ; j <= tamanio_columna ; j++)
                {
                    if(!newn.arreglo[newn.state.est[i].p1.x][newn.state.est[i].p1.y-1] && newn.state.est[i].p1.y-1 >= 1)
                    {
                        newn.arreglo[newn.state.est[i].p1.x][newn.state.est[i].p1.y-1] = true;
                        newn.arreglo[newn.state.est[i].p1.x][newn.state.est[i].p2.y] = false;
                        newn.state.est[i].p1.y--;
                        newn.state.est[i].p2.y--;
                        if(!marcado[newn.state])
                        {
                            movimiento[newn.state].bloque = i;
                            movimiento[newn.state].direccion = j;
                            Q.push(newn);
                            marcado[newn.state] = true;
                        }
                    }
                    else break;
                }
                ...
                for(int j = 1 ; j <= tamanio_columna ; j++)
                {
                    if(!newn.arreglo[newn.state.est[i].p1.x][newn.state.est[i].p2.y+1] && newn.state.est[i].p2.y+1 <= tamanio_columna)
                    {
                        newn.arreglo[newn.state.est[i].p1.x][newn.state.est[i].p1.y] = false;
                        newn.arreglo[newn.state.est[i].p1.x][newn.state.est[i].p2.y+1] = true;
                        newn.state.est[i].p1.y++;
                        newn.state.est[i].p2.y++;
                        if(!marcado[newn.state])
                        {
            ...
            else
            {
                ...
                {
                    if(!newn.arreglo[newn.state.est[i].p1.x-1][newn.state.est[i].p1.y] && newn.state.est[i].p1.x-1 >= 1)
                    {
                        newn.arreglo[newn.state.est[i].p1.x-1][newn.state.est[i].p1.y] = true;
                        newn.arreglo[newn.state.est[i].p2.x][newn.state.est[i].p1.y] = false;
                ...
                for(int j = 1 ; j <= tamanio_fila ; j++)
                {
                    if(!newn.arreglo[newn.state.est[i].p2.x+1][newn.state.est[i].p1.y] && newn.state.est[i].p2.x+1 <= tamanio_fila)
                    {
                        newn.arreglo[newn.state.est[i].p1.x][newn.state.est[i].p1.y] = false;
                        newn.arreglo[newn.state.est[i].p2.x+1][newn.state.est[i].p1.y] = true;
                        newn.state.est[i].p1.x++;
                        newn.state.est[i].p2.x++;
                        if(!marcado[newn.state])
                        {
